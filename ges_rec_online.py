...
gesture_Table = ['< fist >', '< finger_spread >', '< thumb >', '< 1_finger_type >', '< 2_finger_type >', '< V_gesture >', '< relax >','Unknown']
	...
	while(1):
		if(listener.isPrepared):
			...
			if cur_time - start_time > 0.2:
				...
				print(gesture)
				if gesture == 'Unknown':
					data=b'\x55\x55\x14\x03\x05\xC8\x00\x01\xE8\x06\x02\x00\x05\x03\x00\x05\x04\x00\x05\x05\x00\x05'
				else:
				    if gesture == '< finger_spread >':
					    data=b'\x55\x55\x14\x03\x05\xC8\x00\x01\x58\x05\x02\x08\x07\x03\x08\x07\x04\x08\x07\x05\x08\x07'
				    else:
				        if gesture == '< thumb >':
				            data=b'\x55\x55\x14\x03\x05\xC8\x00\x01\x58\x05\x02\xF4\x01\x03\xF4\x01\x04\xF4\x01\x05\xF4\x01'
				        else:
					        if gesture == '< 1_finger_type >':
					            data=b'\x55\x55\x14\x03\x05\xC8\x00\x01\xC4\x09\x02\x08\x07\x03\xF4\x01\x04\xF4\x01\x05\xF4\x01'
					        else:
					            if gesture == '< 2_finger_type >':
					                data=b'\x55\x55\x14\x03\x05\xC8\x00\x01\x58\x05\x02\x08\x07\x03\xF4\x01\x04\xF4\x01\x05\xF4\x01'
					            else:
					                if gesture == '< fist >':
					                    data=b'\x55\x55\x14\x03\x05\xC8\x00\x01\xC4\x09\x02\xF4\x01\x03\xF4\x01\x04\xF4\x01\x05\xF4\x01'
					                else:
					                    if gesture == '< V_gesture >':
					                        data=b'\x55\x55\x14\x03\x05\xC8\x00\x01\xC4\x09\x02\x08\x07\x03\x08\x07\x04\xF4\x01\x05\xF4\x01'
				ser.write(data)
				#print(y_predict.max())
...
def init_model():  
	...
	saver.restore(sess, "./tensorflow_temp/conv_model_w.ckpt" )
